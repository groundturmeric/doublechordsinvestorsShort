...
function draw() {
    ...
    for (var i = 0; i < topCompanies.length; i++) {
        diameter = min(windowWidth, windowHeight) / 3
        topCompanies[i].index_number = i;
        topCompanies[i].x = diameter * Math.cos(radians(((360 / topCompanies.length) * i))) + width / 2;
        topCompanies[i].y = diameter * Math.sin(radians(((360 / topCompanies.length) * i))) + height / 2 - 50;
    }

    for (var i = 0; i < topInvestors.length; i++) {
        diameter = min(windowWidth, windowHeight) / 3
        topInvestors[i].index_number = i;
        topInvestors[i].x = (diameter * 0.5) * Math.cos(radians(((360 / topCompanies.length) * i))) + width / 2;
        topInvestors[i].y = (diameter * 0.5) * Math.sin(radians(((360 / topCompanies.length) * i))) + height / 2 - 50;
    }
